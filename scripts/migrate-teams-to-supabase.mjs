...
/**
 * ============================================================================
 * Migração manual do histórico: teams (SQLite local -> Supabase)
 * ============================================================================
 */
...
if (localTeams.length === 0) {
  console.log("Nada a migrar (nenhum time no banco local).");
  process.exit(0);
}
...
if (!batchError) {
  ...
  for (const row of payload) {
    const { error } = await supabase.from("teams").upsert(row, { onConflict: "id" }).select("id");
    if (error) {
